 ...
 */
function theme_unicundi_get_pre_scss($theme) {
    $scss = theme_unicundi_set_fontwww($css);
    return $scss;
}
 ...
 * @return type|string
 */
function theme_unicundi_get_extra_scss($theme) {
 ...
 * @return string
 */
function theme_unicundi_get_main_scss_content($theme) {
    global $CFG;
    $scss = '';
    $parentconfig = theme_config::load('boost');
    $scss .= theme_boost_get_main_scss_content($parentconfig);
    $themescssfile = $CFG->dirroot.'/theme/unicundi/scss/preset/theme.scss';
 ...
 */
function theme_unicundi_page_init(moodle_page $page) {
    $page->requires->jquery();
    $page->requires->js('/theme/unicundi/javascript/theme.js');
}
...
function theme_unicundi_process_css($css, $theme) {
    global $OUTPUT, $CFG;
    // Set the background image for the logo.
    $logo = $theme->setting_file_url('logo', 'logo');
    $css = theme_unicundi_pre_css_set_fontwww($css);
    // Set custom CSS.
 ...
 */
function theme_unicundi_set_logo($css, $logo) {
    $tag = '[[setting:logo]]';
    $replacement = $logo;
 ...
 * @return bool
 */
function theme_unicundi_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = array()) {
    static $theme;

    if (empty($theme)) {
        $theme = theme_config::load('unicundi');
    }
    if ($context->contextlevel == CONTEXT_SYSTEM) {
        ...
        if ($filearea === 'logo') {
            ...
            theme_unicundi_serve_css($args[1]);
        ...
        } else if ($filearea === 'logoanuncios2') {
            return $theme->setting_file_serve('logoanuncios2', $args, $forcedownload, $options);
        } else if ($filearea === 'logoanuncios3') {
            return $theme->setting_file_serve('logoanuncios3', $args, $forcedownload, $options);
        } else if ($filearea === 'logoanuncios4') {
            return $theme->setting_file_serve('logoanuncios4', $args, $forcedownload, $options);
        } else {
            send_file_not_found();
        }
    } else {
 ...
 * @return string
 */
function theme_unicundi_serve_css($filename) {
    global $CFG;
    if (!empty($CFG->themedir)) {
        $thestylepath = $CFG->themedir . '/unicundi/style/';
    } else {
        $thestylepath = $CFG->dirroot . '/theme/unicundi/style/';
    }
    ...
    if ((($ifmodifiedsince) && (strtotime($ifmodifiedsince) == $lastmodified)) || $etagheader == $etagfile) {
        theme_unicundi_send_unmodified($lastmodified, $etagfile);
    }
    theme_unicundi_send_cached_css($thestylepath, $filename, $lastmodified, $etagfile);
}
 ...
 */
function theme_unicundi_send_unmodified($lastmodified, $etag) {
 ...
 * @param type|string $etag
 */
function theme_unicundi_send_cached_css($path, $filename, $lastmodified, $etag) {
    global $CFG;
 ...
 */
function theme_unicundi_set_customcss($css, $customcss) {
 ...
 */
function theme_unicundi_get_html_for_settings(renderer_base $output, moodle_page $page) {
    global $CFG;
    $return = new stdClass;
 ...
 * @return type|string
 */
function theme_unicundi_pre_css_set_fontwww($css) {
    global $CFG, $PAGE;
    ...
    $tag = '[[setting:fontwww]]';
    $theme = theme_config::load('unicundi');
    $css = str_replace($tag, $themewww.'/unicundi/fonts/', $css);
    return $css;
}
 ...
 * @return type|string
 */
function theme_unicundi_set_fontwww($css) {
    global $CFG, $PAGE;
    if (empty($CFG->themewww)) {
        $themewww = $CFG->wwwroot."/theme";
    } else {
    ...
    $theme = theme_config::load('unicundi');
    $fontwww = '$fontwww: "'.$themewww.'/unicundi/fonts/"'.";\n";
    return $css;
 ...
 * @return type|string
 */
function theme_unicundi_render_slideimg($p, $sliname) {
    global $PAGE, $OUTPUT;
    $nos = theme_unicundi_get_setting('numberofslides');
    $i = $p % 3;
    $slideimage = $OUTPUT->image_url('home/slide'.$i, 'theme');
    // Get slide image or fallback to default.
    if (theme_unicundi_get_setting($sliname)) {
 ...
 * @return type|string
 */
function theme_unicundi_get_setting($setting, $format = false) {
    global $CFG;
 ...
 * @return type|string
 */
function  theme_unicundi_infolink() {
    $infolink = theme_unicundi_get_setting('infolink');
    $content = "";
